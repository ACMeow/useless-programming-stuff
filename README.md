...
前排声明：以下涉及到的所有命令均针对windows环境。Linux玩家大概不需要教，`man diff`就完事了。

TODO：来点表情包。

## 如何使用codeblocks

`Ctrl+Shift+N`新建文件，`Ctrl+S`保存，保存的文件后缀名应为`.c`，`F9`编译并运行当前文件。**不要新建工程！**

建议在设置的编译选项里加入`-Wall -Werror`以显示所有的编译器警告，可以避免一部分变量未初始化，if中使用=等常见错误。

其他请参考群文件《codeblocks简单操作.pdf》 by 凯妹。
...
把你们手里的语法书扔掉，来尝试一下**宇宙最强的[CppReference](https://en.cppreference.com/w/)**。 这个网站收录了非常全面的C语言和C++标准文档，右上角的搜索键可以解决你对一切关键字和一切标准库的疑问（记得尽量选C那边的结果）。悄悄地说其实这个网站支持中文，在页面最底下找找。

象征性给个教学：比如我想把某个变量`x`用`000...000x`这种格式补全到4位数，那么我要怎样寻找这样的功能呢？

1.  首先在右上角[搜索`printf`](https://en.cppreference.com/mwiki/index.php?title=Special%3ASearch&search=printf&button=)；

1.  然后点击[C语言这边的`printf`](https://en.cppreference.com/w/c/io/fprintf)；

1.  然后下拉到Parameters这一段，或者在网页中搜索“format”以寻找和输出格式有关的内容。获得结果如下：

    ![%04d.jpg](img/04d.jpg)

    由此段文档可知，使用%04d即可实现上文的需求。
...
1.  Runtime Error: 运行时错误（包括本地运行时程序崩溃，显示`Process terminated with status 某个反正不是0的数字`的现象），通常有以下几种原因：
    ...
    - 部分OJ在MLE的时候有可能表现为RE（试图访问了未申请到的空间）。
...
1.  首先，由于本地直接运行时输入流、输出流和错误流（[stdin, stdout, stderr][字典]）混在一起，很难分辨出输出中缺失的空格、回车等符号，故先使用[freopen][字典]将输入，输出流和错误流重定向。在main函数开头添加如下代码：
    ...
    分别将输入流重定向到`in.txt`，输出流重定向到`out.txt`，错误流保留在窗口中（也可以将`stderr`用同样的方式重定向）；

1.  在[源文件所在的目录](http://baidu.com/s?wd=windows%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F)下，新建一个文本文档，命名为`in.txt`。将样例**输入**完整的复制到其中，保存；

1.  在源文件所在的目录下，新建一个文本文档，命名为`ans.txt`。将样例**输出**完整的复制到其中，保存；

1.  在源文件所在的目录下，新建一个文本文档，输入如下内容，重命名为`blind.bat`：

    ```bat
    fc out.txt ans.txt /n
    pause
    ```

1.  在本地运行你的程序；

1.  双击运行刚刚创建的blind.bat，即可找到期望输出和实际输出之间的差异。

1.  **提交前务必删掉第一步中添加的三行`freopen`代码！！！**
...
1. QQ自带的截图工具，![QQ Screenshot](img/qq.screenshot.jpg)，也可以使用键盘快捷键Ctrl+Alt+A；之后框选有用的区域，会复制到剪贴板，在聊天窗口粘贴即可。效果：![QQ Screenshot2](img/qq.screenshot2.jpg)
