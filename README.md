...
1. Runtime Error: 运行时错误（包括本地运行时程序崩溃的现象），通常有以下几种原因：
...
1. **代码**：先[对代码进行格式化](#代码风格)。**不要**把代码直接复制粘贴到聊天窗口，**不要**拍屏幕，**不要**对超过20行的代码截图，尽量不要对代码截图。[正确的方式](http://paste.ubuntu.com){:target="_blank"}：（6步）
