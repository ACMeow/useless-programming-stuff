...
其实只是想写一点看起来有点用的东西，回头再整理罢。TODO：来点表情包。
...
编译错误请将codeblocks底部的编译错误信息复制到搜索引擎中寻找答案。实在找不到且无法理解编译错误的内容的话，请[向学长提问](#如何提问)。此节只讨论能编译通过的情况。

### 如何解决bug

此处特指程序运行结果不符合预期的情况。首先，我们需要[准备一组能使程序运行结果不符合预期的输入](#如何找到bug)。既然我们的程序在这组输入数据上不符合预期，那我们想必对该程序的每一部分操作都有一个预期，包括程序的执行流程，各个变量的值的变化等。我们可以用如下方式来分析程序的实际执行流程：
...
#### 程序运行时崩溃/OJ返回RuntimeError

通常有以下几种原因：

1. 访问无效内存（数组越界，使用未初始化或已经释放的指针等）

1. 递归层数过多，耗尽栈空间；

1. 整数除以0。

部分OJ会直接提供RE的错误代码，此时通过搜索引擎可以直接找到对应的原因。
