...
1.  首先，由于本地直接运行时输入流、输出流和错误流（[stdin, stdout, stderr][字典]）混在一起，很难分辨出输出中缺失的空格、回车等符号，故先使用[freopen][字典]将输入，输出流和错误流重定向。在main函数开头添加如下代码：
    ...
    分别将输入流重定向到`D:\in.txt`，输出流重定向到`D:\out.txt`，错误流保留在窗口中（也可以将`stderr`用同样的方式重定向）；

1.  在D盘根目录下，新建一个文本文档，命名为`in.txt`。将样例**输入**完整的复制到其中，保存；

1.  在D盘根目录下，新建一个文本文档，命名为`ans.txt`。将样例**输出**完整的复制到其中，保存；

1.  在D盘根目录下，新建一个文本文档，输入如下内容，重命名为`blind.bat`：
...
在每次逻辑跳转([if, switch, for, while, 函数等][字典])之前和之后，打印一个标记，用来跟踪程序的执行流程，例如：
